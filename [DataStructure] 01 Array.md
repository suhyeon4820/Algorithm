...
>**정의** : 데이터의 원소(Element)를 인덱스(Index)에 대응하도록 구성한 데이터 구조로 <u>논리적 저장 순서와 물리적 저장 순서가 일치</u>

- **특징** : 같은 종류의 데이터를 **순차적으로** 저장하고 효율적으로 관리하기 위해 사용
- **장점** : 빠른 접근 가능 ->인덱스(Index)로 해당 원소(Element)에 접근가능

  - 인덱스의 값만 알고 있으면 `O(1)`에 접근 가능
  -  **Random Access**가 가능
...
- **단점** : 데이터 삽입 / 삭제의 어려움
  ...
  - 해당 원소의 `접근`은 인덱스를 알면 O(1)에 가능하지만 **삽입, 삭제**(마지막 인덱스 제외)의 경우 추가적인 작업이 필요
    - 삽입 : 정렬되어 있는 배열의 중간에 값을 삽입 하려고 하면 값들을 뒤로 밀어야 하기 때문에 `O(n)`이 걸림
    - 삭제 : 배열 중간의 원소를 삭제 할 경우 남은 뒷부분은 앞으로 빈곳을 채우기 위해 `shift`하는 작업이 필요 -> 이때 시간 복잡도는 `O(n)`이 걸림(n == 배열의 사이즈)

- 배열의 삽입, 삭제 시간복잡도 문제를 해결하기 위해 Linked List 사용


| 내용              | 메소드             |
| :---------------- | :----------------- |
| 삽입 - O(1)       | array.append(3)    |
| 삭제 - O(1)       | array.removeLast() |
| 검색, 접근 - O(1) | array[1]           |
| 카운트 & 반환     |                    |
|                   |                    |
|                   |                    |
